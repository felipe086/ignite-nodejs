import { it, beforeAll, afterAll, describe, expect, beforeEach } from 'vitest'
import { execSync } from 'child_process'
import request from 'supertest'
import { app } from '../src/app'

describe('Meals routes', () => {
  beforeAll(async () => {
    await app.ready()
  })

  afterAll(async () => {
    await app.close()
  })

  beforeEach(() => {
    execSync('npm run knex migrate:rollback --all')
    execSync('npm run knex migrate:latest')
  })

  it('should be able to create a new meal', async () => {
    const userResponse = await request(app.server)
      .post('/api/users')
      .send({ name: 'Felipe', email: 'felipe@contato.com' })
      .expect(201)

    const cookies = userResponse.get('Set-Cookie') ?? []

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Salad',
        description: "It's a Salad",
        isOnDiet: true,
        date: new Date(),
      })
      .expect(201)
  })

  it('should be able to list all meals from a user', async () => {
    const userResponse = await request(app.server)
      .post('/api/users')
      .send({ name: 'Felipe', email: 'felipe@contato.com' })
      .expect(201)

    const cookies = userResponse.get('Set-Cookie') ?? []

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Salad',
        description: "It's a Salad",
        isOnDiet: true,
        date: new Date(),
      })
      .expect(201)

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Pizza',
        description: "It's a Pizza",
        isOnDiet: true,
        date: new Date(Date.now() + 24 * 60 * 60 * 1000),
      })
      .expect(201)

    const mealsResponse = await request(app.server)
      .get('/api/meals')
      .set('Cookie', cookies)
      .expect(200)

    expect(mealsResponse.body.meals).toHaveLength(2)

    expect(mealsResponse.body.meals[0].name).toBe('Pizza')
    expect(mealsResponse.body.meals[1].name).toBe('Salad')
  })

  it('should be able to show a single meal', async () => {
    const userResponse = await request(app.server)
      .post('/api/users')
      .send({ name: 'Felipe', email: 'felipe@contato.com' })
      .expect(201)

    const cookies = userResponse.get('Set-Cookie') ?? []

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Salad',
        description: "It's a Salad",
        isOnDiet: true,
        date: new Date(),
      })
      .expect(201)

    const mealsResponse = await request(app.server)
      .get('/api/meals')
      .set('Cookie', cookies)
      .expect(200)

    const mealId = mealsResponse.body.meals[0].id

    const mealResponse = await request(app.server)
      .get(`/api/meals/${mealId}`)
      .set('Cookie', cookies)
      .expect(200)

    expect(mealResponse.body).toEqual({
      meal: expect.objectContaining({
        name: 'Salad',
        description: "It's a Salad",
        is_on_diet: 1,
        date: expect.any(Number),
      }),
    })
  })

  it('should be able to update a meal from a user', async () => {
    const userResponse = await request(app.server)
      .post('/api/users')
      .send({ name: 'Felipe', email: 'felipe@contato.com' })
      .expect(201)

    const cookies = userResponse.get('Set-Cookie') ?? []

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Salad',
        description: "It's a Salad",
        isOnDiet: true,
        date: new Date(),
      })
      .expect(201)

    const mealsResponse = await request(app.server)
      .get('/api/meals')
      .set('Cookie', cookies)
      .expect(200)

    const mealId = mealsResponse.body.meals[0].id

    await request(app.server)
      .put(`/api/meals/${mealId}`)
      .set('Cookie', cookies)
      .send({
        name: 'Sushi',
        description: "It's a Sushi",
        isOnDiet: true,
        date: new Date(),
      })
      .expect(204)
  })

  it('should be able to delete a meal from a user', async () => {
    const userResponse = await request(app.server)
      .post('/api/users')
      .send({ name: 'Felipe', email: 'felipe@contato.com' })
      .expect(201)

    const cookies = userResponse.get('Set-Cookie') ?? []

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Salad',
        description: "It's a Salad",
        isOnDiet: true,
        date: new Date(),
      })
      .expect(201)

    const mealsResponse = await request(app.server)
      .get('/api/meals')
      .set('Cookie', cookies)
      .expect(200)

    const mealId = mealsResponse.body.meals[0].id

    await request(app.server)
      .delete(`/api/meals/${mealId}`)
      .set('Cookie', cookies)
      .expect(204)
  })

  it('should be able to get metrics from a user', async () => {
    const userResponse = await request(app.server)
      .post('/api/users')
      .send({ name: 'Felipe', email: 'felipe@contato.com' })
      .expect(201)

    const cookies = userResponse.get('Set-Cookie') ?? []

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Salad',
        description: "It's a Salad",
        isOnDiet: true,
        date: new Date('2021-01-01T08:00:00'),
      })
      .expect(201)

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Pizza',
        description: "It's a Pizza",
        isOnDiet: false,
        date: new Date('2021-01-01T12:00:00'),
      })
      .expect(201)

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Soda',
        description: "It's a Soda",
        isOnDiet: true,
        date: new Date('2021-01-01T15:00:00'),
      })
      .expect(201)

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Sushi',
        description: "It's a Sushi",
        isOnDiet: true,
        date: new Date('2021-01-01T20:00:00'),
      })

    await request(app.server)
      .post('/api/meals')
      .set('Cookie', cookies)
      .send({
        name: 'Salad',
        description: "It's a Salad",
        isOnDiet: true,
        date: new Date('2021-01-02T08:00:00'),
      })

    const metricsResponse = await request(app.server)
      .get('/api/meals/metrics')
      .set('Cookie', cookies)
      .expect(200)

    expect(metricsResponse.body).toEqual({
      totalMeals: 5,
      totalMealsOnDiet: 4,
      totalMealsOffDiet: 1,
      bestOnDietSequence: 3,
    })
  })
})
